...
const Navbar = () => {
        ...
        if (!user.auth) {
            ...
            return (
                <>
                    ...
                    <Link className="nav-item" to='/card-listing'><div>Listing</div></Link>
                    <Link className="nav-item" to='/submit-card'><div>Submit Card</div></Link>
                    <Link className="nav-item" to='/logout'><div>Logout</div></Link>
                </>
            )
        }
    }
    ...
    return (<>
        <div className="">
            <div className="sm:hidden w-screen" onClick={() => {setMobile(!mobile)}}>
                <svg className="sm:hidden w-1/12 absolute right-0 m-4"
                     xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16" />
                </svg>
            </div>
            {mobile && (<div className="flex flex-col sm:hidden transition duration-500 mobile-nav absolute mt-12" onClick={()=>{setMobile(false)}}>
                { getNav() }
            </div>)}
